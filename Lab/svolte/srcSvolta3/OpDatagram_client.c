...
int main(int argc, char **argv) {
     ...
     */
    clientaddr.sin_port = 0;        // posso provare a fare bind(), finchè non trovo una porta libera?
    ...
    while ((ok = scanf("%i", &num1)) != EOF) {
        ...
        do {
            ...
            c = getchar();      //qua succede qualcosa di strano con le stringhe
            gets(okstr); // consumo resto linea
        } while (c != '+' && c != '-' && c != '*' && c != '/');
        ...
        /* lettura completata */
        //qua la ntoh non serve perchè le variabili mi sono arrivate da host non da network
