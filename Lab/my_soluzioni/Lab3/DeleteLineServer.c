...
int main(int argc, char **argv) {
    ...
    for (;;) {
        ...
        if (fork() == 0) { // figlio
            ...
            printf("Server (figlio): eseguo l'eliminazione della linea\n");

            /* SERVIZIO */
            /*RICEVO numero di linea*/
            printf("Server (figlio): ricevo e stampo senza linea richiesta\n");
            if ( read(conn_sd, &numLinea, sizeof(int)) < sizeof(int) ) {     //TODO: ripeti delle read finchè non leggi sizeof(int) byte
                perror("read linea da eliminare");
                continue;
            }
            printf("Server(figlio): devo eliminare la linea %d!\n", numLinea);
            /*RICEVO e RIMANDO indietro il File senza la linea richiesta*/
            contaLinea=1;

            while ( (nread = read(conn_sd, &buff, 1)) > 0 ) {
                if( contaLinea != numLinea ) {
