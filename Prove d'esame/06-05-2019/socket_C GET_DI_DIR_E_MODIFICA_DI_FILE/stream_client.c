...
int main(int argc, char *argv[]) {
    ...
    char char_buf;
    char nome_dir[LINE_LENGTH], nome_file[LINE_LENGTH];   
    int len;
    int fd;
    int error;
    int contaLetti = 0;
    ...
    printf("Inserire il nome del direttorio da trasferire: ");
    while (gets(nome_dir)) {
        printf("\trecupero: %s\n", nome_dir);

        /* invio nome directory/file */
        nwrite = write(sd, nome_dir, (strlen(nome_dir) + 1) );
